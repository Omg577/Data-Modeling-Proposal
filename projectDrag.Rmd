...
library(dplyr)
library(broom)
library(ggplot2)  # <--- Added this so your graphs work!

# --- Data Loading -----------------------------------------------------------
# Check where R is looking (Output appears in console/markdown)

df <- read.csv("student.csv", sep = ";")

# --- Data Cleaning ----------------------------------------------------------
# Use mutate instead of transmute to keep original data available if needed
df <- df %>%
  mutate(
...
# H0: mean(G3 | has_failure=0) = mean(G3 | has_failure=1)
means_by_group <- df %>%
...
print(t_out) # Added print so you see the result in the knitted doc
